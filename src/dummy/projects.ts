...
패션브랜드 소개에 인터렉티브한 요소를 추가해 흥미를 더 한 프로젝트입니다.`,
...
하지만, 프로젝트가 조금은 흐지부지 하게 끝난점이 있다고 생각되어, 아쉬움이 많이 남는 프로젝트이기도 합니다.`,
    function: `반려동물 커뮤니티로, 정보 및 일상에 대한 글을 등록 및 삭제할 수 있는 사이트입니다. 댓글 기능이 있어 게시글 내에서 사용자들끼리 소통을 할 수 있고,
...
회원가입 시 의뢰자 또는 디자이너 중에 직업을 선택할 수 있고,
의뢰자로 직업을 선택할 시 게시글을 작성하여 디자이너에게 커미션을 부탁할 수 있고,
디자이너는 의뢰자의 글에 커미션을 받아서 의뢰를 처리할 수 있습니다.
게시글을 작성할때는 사진을 공개 또는 비공개로 처리할 수 있으며, 비공개로 처리할 시 사진은 의뢰를 받은 디자이너 1명에게만 보여지게 처리하였습니다.
...
팀원은 프론트3, 백엔드1 로 구성되어 진행하였고,
AWS EC2 에 배포완료 후, 실제 운영 예정이였으나, 해당 사이트에서는 SSR 을 사용하지 않으므로
...
옷을 등록한 후, 오늘날짜나 과거날짜에 무엇을 입었는지 옷장에 있는 옷과 오늘의 코디사진을 함께 업로드하고 후기 및 별점을 남겨서
어떤 날씨 및 온도에 무엇을 입고갔는지 일기장처럼 적어놓고, 다음에 어떤옷을 입어야할지 참고로 쓰는 나만의 일기장 같은 사이트입니다.
메인 페이지에서는 오늘 날씨와 가장 유사한 순서대로 과거데이터들을 나열해주고,
달력 페이지에서는 코디를 등록한 날짜와 날씨 및 온도를 한눈에 확인할 수 있으며, 클릭 시 그날의 코디를 볼 수 있게 해놓았습니다.

※ 메인페이지에 날씨 이모지를 클릭하면 3D 모델링을 볼 수 있는 이스터에그를 숨겨놓았습니다.`,
    developEnv: [
      'Next.js',
      'TypeScript',
      'Emotion',
  ...
  {
    slideImage: 'street-vendor.jpg',
    title: '손안의 노점',
    people: '프론트 3, 백엔드 1, 디자이너2',
    date: ['2023.01', 'ing'],
    imageUrl: ['street-vendor.jpg', 'street-vendor.jpg'],
    info: `대부분 길찾기 어플에는 붕어빵, 타코야끼 등 여러 포장마차의 대한 정보가 나오지 않는데,
이러한 점을 공략하여 시작하게 된 프로젝트입니다. 디자이너와 실제로 XD 및 Figma 를 통해 협업 진행중입니다.`,
    function: `손님은 붕어빵, 타코야끼 등의 노점을 쉽게 찾아서 좋고,
사장님은 이 어플을 통해 손님을 좀 더 유입시킬 수 있다는 점이 이 앱의 핵심 기능입니다.
구글 로그인으로 로그인을 할 수 있으며, 사장앱, 손님앱을 분리하여 작업할 계획이며,
손님들이 가게가 있다고 제보하는것이 아닌, 사장님이 직접 가게를 등록함으로써
손님들이 앱의 정보를 보고 갔다가 가게가 없는 상황을 방지하였습니다.
`,
